...
import { Save, CheckCircle, AlertCircle, User, Info, Mail } from 'lucide-react';
    ...
    <div className="space-y-8 max-w-4xl">
      <div>
        <h1 className="text-3xl font-extrabold uppercase tracking-tight text-white flex items-center gap-3">
          ...
          <span>EDIT TENTANG SAYA <span className="text-amber-400">(ABOUT & BIO)</span></span>
        </h1>
        <p className="text-xs text-gray-400 mt-1">
          Edit paragraf deskripsi diri (bio), foto avatar/profil, serta alamat kontak dan media sosial Anda.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-black p-6 sm:p-8 rounded-2xl border border-gray-800 space-y-6 shadow-xl">
        ...
        <div className="space-y-4">
          <h2 className="text-sm font-extrabold uppercase tracking-wider text-amber-400 border-b border-gray-800 pb-2 flex items-center gap-2">
            <Info size={18} />
            <span>1. DESKRIPSI DIRI & AVATAR</span>
          </h2>

          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-gray-300 mb-1">
              PARAGRAF BIO (DESKRIPSI DIRI)
            </label>
            <textarea
              rows={4}
              value={profile.bio}
              onChange={(e) => setProfile({ ...profile, bio: e.target.value })}
              placeholder="Tuliskan latar belakang, minat, dan fokus keahlian Anda..."
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-gray-300 mb-1">
              URL FOTO AVATAR / PROFIL (MENDUKUNG GDRIVE & LINK DIRECT)
            </label>
            <input
              type="text"
        ...
        <div className="space-y-4 pt-4">
          <h2 className="text-sm font-extrabold uppercase tracking-wider text-amber-400 border-b border-gray-800 pb-2 flex items-center gap-2">
            <Mail size={18} />
            <span>2. DETAIL KONTAK & MEDIA SOSIAL</span>
          </h2>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setProfile({
                    ...profile,
                    contactInfo: { ...profile.contactInfo, instagram: e.target.value },
                  })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setProfile({
                    ...profile,
                    contactInfo: { ...profile.contactInfo, linkedin: e.target.value },
                  })
                }
        ...
        {status === 'success' && (
          <div className="flex items-center space-x-2 text-green-400 bg-green-950/60 border border-green-800 p-3 rounded-lg text-xs font-bold">
            <CheckCircle size={18} />
            <span>Perubahan Tentang Saya berhasil disimpan!</span>
          </div>
        )}
        ...
        >
          <Save size={16} />
          <span>{saving ? 'MENYIMPAN...' : 'SIMPAN PERUBAHAN TENTANG SAYA'}</span>
        </button>
      </form>
    </div>
  );
